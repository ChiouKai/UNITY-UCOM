...
public class LogoScript : MonoBehaviour
{
    ...
    {
        if ((RT.anchoredPosition- InsertLocation).magnitude > 0.2f)
        {
    ...
    {
        if ((RT.anchoredPosition - LeaveLocation).magnitude > 0.2f)
        {
    ...
    public void LogoDeath()
    {
        if ((RT.anchoredPosition - LeaveLocation).magnitude > 0.2f)
        {
            RT.anchoredPosition = Vector2.Lerp(RT.anchoredPosition, LeaveLocation, 0.07f);
        }
        else
        {
            TimeLine.Instance.Moved = true;
